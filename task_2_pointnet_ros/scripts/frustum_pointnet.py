...
class FrustumPointNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_dont_care_mask = out_dont_care_mask.type(torch.ByteTensor).cpu() # (NOTE! ByteTensor is needed for this to act as a selction mask)
        ...
        out_seg_point_clouds_mean = Variable(torch.from_numpy(seg_point_clouds_mean)).cpu()
        ...
        seg_point_clouds = Variable(torch.from_numpy(seg_point_clouds)).cpu() # (shape: (batch_size, num_seg_points, 3))
